...
namespace TiltBrush
{
    ...
    public class ControllerGeometry : MonoBehaviour //控制几何结构
    {
        // -------------------------------------------------------------------------------------------- //
        // Inspector Data
        // -------------------------------------------------------------------------------------------- //
        ...
        [SerializeField] private Renderer m_TriggerMesh;//右手柄按钮
        ...
        //如果锚点与其子锚点之间存在任何偏移，则很难调整此数字，              
        //因为刻度放大了偏移量。最简单的办法就是不要抵消              
        //这意味着弹出窗口的高度完全由PadPopUpAmount指定.   
        //请注意，此值当前用于拇指棒和触摸板弹出窗口。
        ...
        // Wmr controller components.//Wmr控制器组件。
        ...
        //样式本来是只读和不可变的，但目前有一种情况              
        //这就要求它是可写的。TODO:尽可能删除？
        ...
        //在与此几何体相关联的行为之后调用（即转换.parent)变化。
        // Only really for use by BaseControllerBehavior.
        //只供BaseControllerBehavior使用。
        ...
        // Show the undo/redo icons, conditionally based on undo availability./根据撤消可用性有条件地显示撤消/重做图标。
