...
namespace TiltBrush
{
    ...
    public class PanelManager : MonoBehaviour
    {
        ...
        //查看者是否在简介草图中的代理（第二次运行及以后）。              
        //TODO:将此属性移到应用程序.cs更合理的地方。
        ...
        public void SetVisible(bool bVisible)
        {
            ...
            {
                // Prep panels for exit if they're enabled.//如果启用了面板，请准备退出面板。
        ...
        public void ToggleSketchbookPanels(bool isLoadingSketch = false)
        {
            ...
            //我们只希望在第一次启动程序时直接从“库”面板加载草图时默认为“颜色选择器”。
            ...
            //始终将其设置为false，因为我们将新草图作为第一个草图加载。
        ...
        //此函数在“mode”参数和PanelMode.标准.  目前，不允许从非标准模式转换到另一个非标准模式允许.toModefromMode定义了到mode的过渡模式。
        void ToggleMode(List<BasePanel> panels, PanelMode mode, PanelMode toMode, PanelMode fromMode)
        {
            if (panels.Count > 0)
            {
                // In "standard" mode, transition to target mode.//在“标准”模式下，转换到目标模式。
                if (m_PanelsMode == PanelMode.Standard ||
