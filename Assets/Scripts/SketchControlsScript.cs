...
namespace TiltBrush
{
    ...
    public class SketchControlsScript : MonoBehaviour
    {
        ...
        //这是用来计算mïu CurrentGazeHitPoint的凝视。
        //它不是一般的替代品ViewpointScript.凝视.
        /// <summary>
        /// 当前从头部发出的射线
        /// </summary>
        ...
        public void UpdateControls()
        {
            ...
            // Verify controllers are available and prune state if they're not.//验证控制器是否可用，如果不可用，则删除状态。
            if (App.VrSdk.GetControllerDof() == VrSdk.DoF.Six &&
        ...
        {
            // If the pin cushion is showing and the user cancels, eat the input.//如果显示了pin缓冲并且用户取消了，则吃掉输入。
            ...
            //如果我们的工具想要阻止输入，那么保持输入eat状态直到              //在用户释放输入之后。
        ...
        {
            // Are the panels allowed to be visible?//面板是否允许可见？
            ...
            {
                // Transition panels according to requested visibility.//根据要求的能见度设置过渡板。
        ...
        void UpdateBaseInput()
        {
            ...
            bool bScaleCommandActive =
              ...
              && bScaleInputActive
              && m_CurrentGazeObject == -1   // free up swipe for use by gaze object//释放滑动供凝视对象使用
        ...
        void UpdateGrab_WasNone(GrabWidget rPrevPotentialBrush, GrabWidget rPrevPotentialWand)
        {
            ...
            //如果面板不在焦点中，允许小部件抓取              //我们可以抓住一个小部件，只要我们不试图用那只手画。
            bool bActiveInput =
              (InputManager.m_Instance.GetCommand(InputManager.SketchCommands.Activate) &&
               App.Instance.IsInStateThatAllowsPainting());
            ...
            if (m_CurrentInputState == InputState.Standard && bWidgetManipOK)
            {
                ...
                if (m_BackupBrushGrabData != null)
                {
                    ...
                    // Allow widget grab if we're not painting.//如果我们不画画就允许抓取。
                    if (!bActiveInput)
                    {
