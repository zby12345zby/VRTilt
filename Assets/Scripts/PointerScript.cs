...
namespace TiltBrush
{
    ...
    public class PointerScript : MonoBehaviour
    {
        ...
        //this is the list of meshes that make up the standard pointer look: cone + ring//这是组成标准指针外观的网格列表：圆锥体+环
        ...
        //这是一个脆弱的函数。仅当在工具移动指针之后和调用UpdatePointer（）之前调用时，它才有效。
        ...
        /// TODO: see above.
        /// ///返回相对于传递的行变换的xf\u RS。              ///应用m\u LineDepth并忽略xf_比例尺              ///TODO:见上图。
        ...
        public void UpdatePointer()
        {
            ...
            if (m_CurrentLine != null)
            {
                // Non-preview mode: Update line with new pointer position//非预览模式：用新指针位置更新行
        ...
        public void UpdateLineFromObject()
        {
            ...
            //更新笔划位置点
            bool bQuadCreated = m_CurrentLine.UpdatePosition_LS(xf_LS, m_CurrentPressure);//xf_LS传递的是pointscript游戏对象的坐标

            // Debug.Log("笔划压力为：" + m_CurrentPressure);//测试时m_CurrentPressure==1；


            // TODO: let brush take care of storing control points, not us //TODO:让brush负责存储控制点，而不是我们
            SetControlPoint(xf_LS, isKeeper: bQuadCreated);  //感觉像是存储已画出的线条的代码(在绘制结束时显示它们)
            ...
            // the active stencil.
            //TODO:指针应该包含对正在绘制的模板的引用。这是一个黑客临时检查，以确保镜像指针不会增加活动模具的提升。

            if (PointerManager.m_Instance.MainPointer == this)
            {
                // Increase stencil lift if we're painting on one. //如果我们在一个上面画画的话，增加模板的提升。
            ...
            UpdateLineVisuals();   //个人认为应该是把线条显示出来的作用（边画边显示出来）
        ...
        /// Bulk control point addition///批量控制点添加
        ...
        public void DisablePreviewLine()
        {
            if (m_PreviewLine)
            {
                ...
                //TODO:删除对DestroyMesh（）的这个（和其他）调用，改为在OnDestroy（）中执行？
        ...
        /// ///将转换记录为控制点。
        /// ///如果最近的点是keeper，则附加一个新的控制点。
        /// ///否则，最近的一点是keeper，并且将被覆盖。
        /// ///              ///参数“keep”指定新写入的点是否为keeper。 
        /// ///              ///查询/不查询当前指针以获取新指针的转换 
        /// ///控制点。相反，调用者负责传递相同的消息 
        /// ///传递给的xfline.UpdatePosition\u LS()
        ...
        //传递画布父对象，并在画布的空间中进行变换。
        //如果overrideDesc通过，则将其用于视觉效果 —— m_CurrentBrush不会更改。
        /// <summary>
        /// 创建新线
        /// </summary>
        /// <param name="canvas"></param>
        /// <param name="xf_CS"></param>
        /// <param name="creator"></param>
        /// <param name="overrideDesc"></param>
                                  ...
                                  ParametricStrokeCreator creator, BrushDescriptor overrideDesc = null)
        {
            ...
            // Parametric creators want control over the brush size.//参数化创建者希望控制笔刷大小。
        ...
        /// ///比如BeginLineFromMemory+EndLineFromMemory
        /// ///为了帮助捕获更高级笔划代码中的错误，需要考虑
        /// ///错误，除非笔划处于未创建状态。
        ...
        ///在回放期间，rMemoryObjectForPlayback为非零，不应传递笔划标志。
        ///否则，rMemoryObjectForPlayback为零，笔划标志应有效。
        ///非零时，rMemoryObjectForPlayback对应于当前行。
            ...
            SketchMemoryScript.StrokeFlags strokeFlags = SketchMemoryScript.StrokeFlags.None)
        {
            ...
            //会被执行
            else if (App.Config.m_UseBatchedBrushes && m_CurrentLine.m_bCanBatch)//App.Config.m_UseBatchedBrushes == true
            {
                ...
                //ClearRedo（）由memorexxx（）调用，但也可以作为碎片优化在前面进行；这允许GenerateBatchSubset（）重用回收的空间。
                //注意：批处理目前处理碎片的能力很差，因此认为这种优化是必要的。
                if (rMemoryObjectForPlayback == null)
                {
                ...
                //destroy original stroke, as it's now part of the batch stroke//销毁原始笔划，因为它现在是批笔划的一部分
                m_CurrentLine.DestroyMesh();
                Destroy(m_CurrentLine.gameObject);

                // recreate the stroke if it's just been drawn by the user, so we can run the simplifier on it.
                //重新创建笔划，如果它只是由用户绘制的，这样我们就可以运行它的简化。
            ...
            else//调试时不会被执行
            {
                ...
                //copy master brush over to current line//将主画笔复制到当前行
                m_CurrentLine.FinalizeSolitaryBrush();
            }
            ...
            m_CurrentLine = null;
            //Restore brush size if our parametric creator had to modify it.//恢复笔刷大小，如果我们的参数化创建者必须修改它。
            if (m_CurrentCreator != null)
            {
        ...
        //如果线应该保留或具有戳记实现，则返回false              //否则返回true
